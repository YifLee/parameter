...
def trainerThread (s2c, c2s, e,
                    ...
                    debug=False,
                    **kwargs):
    if isinstance(saving_time,float) and (0.5 <= saving_time <= 11):
        saving_m = saving_time * 60
    else:
        saving_m = 30
    while True:
        try:
            ...
            if model.get_target_iter() != 0:
                if is_reached_goal:
                    ...
                    #         continue
                    # if iter_time >= 10:
                    #     loss_string = int('{:.5s}'.format('{:0.4f}'.format(iter_time)))
                    # else:
                    #     loss_string = int('{:04d}'.format(int(iter_time*1000))) / 1000
                    # target_iter = model.get_target_iter()
                    # iter_nums = target_iter - iter
                    # train_time = iter_nums * loss_string
                    # time_h = int(train_time // 3600)
                    # if time_h != 0:
                    #     time_m = int((train_time / 3600) % time_h)
                    # else:
                    #     time_m = int((train_time / 3600) * 60)
                    # if time_m + 10 >= 60:
                    #     time_h += 1
                    #     time_m = time_m + 10 - 60
                    # train_time = train_time + 600
                    io.log_info('开始训练. 目标迭代: %d' % (model.get_target_iter()))
            else:
                try:
                    if saving_m <60:
                        io.log_info('开始训练. %d分钟后完成' % (saving_m))
                    elif saving_m % 60 == 0:
                        io.log_info('开始训练. %d小时后完成' % (saving_m // 60))
                    elif saving_m > 60:
                        io.log_info('开始训练. %d小时%d分钟后完成' % (saving_m // 60 , (saving_m - (saving_m // 60)*60)))
                except:
                    io.log_info('开始训练.')
            ...
            for i in itertools.count(0,1):
                if not debug:
                    ...
                    if not is_reached_goal:
                        ...
                        delta = t1_time - t0_time
                        delta_gmtime = time.gmtime(delta.total_seconds())
                        duration_str = time.strftime("%H:%M", delta_gmtime)
                        h_m = duration_str.split(':')
                        duration_m = int(h_m[0]) * 60 + int(h_m[-1])
                        if duration_m >= saving_m:
                            io.log_info('已到达定时时长.')
                            model_save()
