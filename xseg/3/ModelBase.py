...
class ModelBase(object):
    ...
    def ask_override(self):
        return self.is_training and self.iter != 0 

    def ask_autobackup_hour(self, default_value=0):   #果子狸修改
        default_autobackup_hour = self.options['autobackup_hour'] = self.load_or_def_option('autobackup_hour', default_value)
        self.options['autobackup_hour'] = 0         #备份时间，整数。建议0，在kaggle中可以手动备份！！

    def ask_write_preview_history(self, default_value=False):    #果子狸修改
