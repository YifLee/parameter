...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        default_masked_training    = self.options['masked_training']    = self.load_or_def_option('masked_training', False)
...
""").lower()
        ...
        if self.is_first_run() or ask_override:
            self.options['models_opt_on_gpu'] = True    #将模型添加到GPU，True-放入，False-不放入，二选一，建议True
            ...
            if self.options['gan_power'] != 0.0:        #不需要修改
                ...
                self.options['gan_patch_size'] = default_gan_patch_size
            ...
            if 'df' in self.options['archi']:
                self.options['true_face_power'] = 0.01   #真脸(使结果更像SRC)，按需输入数值，建议0.001-0.01

                self.options['true_face_power'] = 0.01   #照搬上面
